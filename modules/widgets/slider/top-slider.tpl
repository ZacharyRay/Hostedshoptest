...
Modifications: line 213 - cropping removed from the src of the slider image in order to make it full width.
    ...
    {foreach $files as $item}
        ...
        {/if}
            <figure class="item" data-fileid="{$item->Id}">
                ...
                {*** Use width and height to generate a thumbnail ***}
                {$itemSrc = $item->thumbnail(null, null, null, $watermark)|solutionPath}
                ...
                {if $sliderHasOwl}
                    {if $item@index == 0}
                        <img class="w-slider-img" src="{$itemSrc}" alt="{$itemAlt}">
                    {else}
                        {img class="w-slider-img" src="{$itemSrc}" alt="{$itemAlt}" slider=true}
                    {/if}
                {else}
                    {if is_numeric($itemHeight) && is_numeric($itemWidth)}
                        {$itemHeight = (int)$itemHeight}
                        {$itemWidth = (int)$itemWidth}
                        <span class="placeholder-wrapper" data-placeholder-max-width="{$itemWidth}px">
                            <span class="placeholder" data-placeholder-aspect="{($itemHeight / $itemWidth) * 100}%"></span>
