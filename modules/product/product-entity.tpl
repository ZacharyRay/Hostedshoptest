...
Modifications:
Line 127: Moved form wrapper furthing into nesting
Line 459: Moved short description down below actions
Line 469: Moved varianttypes orderlist and orderlist-dropdown up into right column
Line 498: Moved product additional info up into right column
...
<div class="modules m-product trailing-trip" itemscope itemtype="http://schema.org/Product" data-controller="{$item->Id}">
    ...
    {if $category && $category->Title}
        {addMeta property="product:category" content=$product->Category}
                            ...
                            {if !$item->CallForPrice}
                                ...
                                {foreach $priceData as $priceDataInterval}
                                    ...
                                    <span class="is-block m-product-priceline">
                                        ...
                                        <span class="h4 m-product-price {if $priceDataInterval->PriceMin lt $priceDataInterval->FullPriceMin}price-after-discount{/if}" >{$priceDataInterval->PriceMin|formatPrice}</span>
                                ...
                                <p>
                                    ...
                                    {if $settings.shop_product_tax_after_price or (!$currency.hasVat and $settings.shop_product_lineprice_vat)}
                                        {if $currency.hasVat or $settings.shop_product_lineprice_vat}
                                            <span class="m-product-price-vat-text has-vat">{$text.PRODUCT_CATALOG_PRODUCT_PRICE_TAX}</span>
                                        {else}
                                            <span class="m-product-price-vat-text has-novat">{$text.PRODUCT_CATALOG_PRODUCT_PRICE_TAX_NO}</span>
                                        {/if}
...
{* Productlist:Related Products *}
    {collection assign=relatedProducts controller=productRelated productId=$item->Id orderBy='-Sold,Sorting,Title'}
