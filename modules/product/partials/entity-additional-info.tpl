...
Modifications:
- Moved all tab content up into list items, in order to create an accordion, rather than a tabset
...
<div class="m-product m-product-additional-info{if !$hasTabs} is-hidden" data-controller="description--wrap{/if}">
    ...
    <ul class="nav nav-tabs m-product-additional-info-tabs">
        ...
        {if $hasExtraBuy}
            <li class="active m-product-additional-info extra-buy">
                <a class="tab-link" style="cursor:pointer;" data-toggle="content" data-target="#tabs-pane0">{$text.PRODUCT_CATALOG_EXTRABUY}</a>
        ...
        {*** Tab nav: Description ***}
        <li class="{if (!$hasActive && !empty($longDesc)) || !$hasTabs}active{/if} m-product-additional-info description{if empty($longDesc)} is-hidden" data-controller="description--wrap{/if}">
            <a class="tab-link" style="cursor:pointer;" data-target="#tabs-pane1" data-toggle="content">{$text.DESCRIPTION}</a>
            ...
            <div class="pane {if $hasActive and $active==="description"}active{/if} m-product-additional-info description clearfix{if empty($longDesc)} is-hidden" data-controller="description--wrap{/if}" id="tabs-pane1" itemprop="description">
                <div data-controller="description">{$longDesc}</div>
        ...
        {if $hasCustomData and $access.customData}
            <li class="{if !$hasActive}active{/if} m-product-additional-info customdata">
                <a class="tab-link" style="cursor:pointer;" data-target="#tabs-pane2" data-toggle="content">{$text.SPECIFICATIONS}</a>
        ...
        {if $productPdfs->getActualSize() gt 0}
            <li class="{if !$hasActive}active{/if} m-product-additional-info files">
                <a class="tab-link" style="cursor:pointer;" data-target="#tabs-pane3" data-toggle="content">{$text.DOWNLOADS}</a>
        ...
        {if $settings.module_shop_review_products_type!=="stars" and $settings.module_shop_review_products and $access.reviews}
            <li class="{if !$hasActive}active{/if} m-product-additional-info review">
                <a class="tab-link" style="cursor:pointer;" data-target="#tabs-pane4" data-toggle="content">{$text.PRODUCT_CATALOG_REVIEW_ADD_REVIEWS}</a>
        ...
        <li class="{if !$hasActive}active{/if} m-product-additional-info tags">
            <a class="tab-link" style="cursor:pointer;" data-target="#tabs-pane5" data-toggle="content">{$text.TAGS}</a>
        ...
        {if $settings.social_facebook or $settings.social_twitter or $settings.social_google}
        <li class="{if !$hasActive}active{/if} m-product-additional-info social">
            <a class="tab-link" style="cursor:pointer;" data-target="#tabs-pane6" data-toggle="content">{$text.SOCIAL_MEDIA}</a>

            {if $settings.social_facebook or $settings.social_twitter or $settings.social_google}
                <div class="pane {if $hasActive and $active==="social"}active{/if} m-product-additional-info social" id="tabs-pane6">
                    <div class="row">
                        <div class="col-s-4 col-m-6 col-l-6 col-xl-12">
                            <p><span class="h6">{$text.SOCIAL_MEDIA}</span></p>
                        </div>
